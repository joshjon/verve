		...
		<form onsubmit={handleSubmit}>
			<div class="py-4 space-y-7">
				<div class="space-y-3">
					...
					<label
						for="not-ready"
						class="flex items-start gap-2 cursor-pointer text-sm transition-colors -mb-1"
					...
					>
						<input
							...
							class="w-3.5 h-3.5 rounded border-input accent-orange-500 mt-0.5"
							disabled={loading}
						/>
						<div>
							<span>Mark as not ready</span>
							<span class="block text-xs text-muted-foreground">Won't run until marked as ready</span>
						</div>
					</label>
