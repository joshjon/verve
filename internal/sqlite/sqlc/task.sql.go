...
func (q *Queries) DeleteExpiredLogs(ctx context.Context, createdAt time.Time) (int64, error) {
...
func (q *Queries) ListStaleTasks(ctx context.Context, lastHeartbeatAt *string) ([]*Task, error) {
...
SELECT id, repo_id, description, status, pull_request_url, pr_number, depends_on, close_reason, attempt, max_attempts, retry_reason, acceptance_criteria, agent_status, retry_context, consecutive_failures, cost_usd, max_cost_usd, created_at, updated_at, skip_pr, branch_name, title, acceptance_criteria_list, model, started_at, ready, last_heartbeat_at, epic_id FROM task ORDER BY created_at DESC
`
