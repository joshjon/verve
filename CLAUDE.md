...
- **Do not run Playwright screenshot tests locally.** The Playwright screenshot tests (`ui/e2e/screenshots.spec.ts`) are designed to run only in GitHub Actions CI, which has a full browser environment. Agent and headless Linux environments cannot run a browser, so these tests will fail locally. Only add or modify the test code — CI will capture the actual screenshots automatically on PRs that touch `ui/`.
...
- `sql.ErrNoRows` → `ErrTagTaskNotFound`
